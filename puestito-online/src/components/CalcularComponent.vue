<template>
  <div class="container mt-2 col-md-6">
    ...
    <div class="border p-2 mt-2">
      ...
      <div>
        <button class="btn btn-primary mt-2" :disabled="!ocrResult" @click="convertirATextoAVoz">Descargar
          Audio</button>
      </div>
      <div v-if="descargando">Descargando...</div>
    </div>
    <div class="border p-2 mt-2">
      <h3>Convertir audio en texto:</h3>
      <hr>
      <input class="form-control" type="file" @change="manejarSubidaAudio">
      <div v-if="loading2">
        <div class="text-center">Procesando...</div>
      </div>
      <div v-if="transcripcion">
        <p class="text-center mt-2">{{ transcripcion }}</p>
      </div>
      <div>
        <button class="btn btn-primary mt-2" @click="descargarTexto" :disabled="!transcripcion">Descargar Texto</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async manejarSubidaAudio(event) {
      ...
      if (file) {
        ...
        try {
          ...
          this.transcripcion = response.data.transcription;
        } catch (error) {
    ...
    descargarTexto() {
      ...
      const file = new Blob([this.transcripcion], { type: 'text/plain' });
